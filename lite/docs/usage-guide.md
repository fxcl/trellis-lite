...
Trellis Lite 支持以下 AI 编码工具，核心工作流完全相同，只是入口文件和 agents/commands 分发方式不同：

| 平台 | 入口文件 | 安装的 agents / commands | 安装参数 |
|---|---|---|---|
...
核心工作流（PLAN → CODE → WRAP）和 `trellis.py` CLI 在所有平台完全一致。差异仅在分发方式：
